...
describe('list module', () => {
  describe('#list()', () => {
    ...
    it('should list all words whose part of speech starts with given string', () => {
      ...
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.startsWith('v')).to.be.true
      )
    })
    // list pos ends m.
    it('should list all words whose part of speech ends with given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.endsWith('m.')).to.be.true
      )
    })
    // list pos has vin.
    it('should list all word whose part of speech contains given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.includes('vin.')).to.be.true
      )
    })
    // list pos is v.
    ...
    it('should list all word whose part of speech is like the given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.match(/v.*\./g).length > 0).to.be.true
      )
    })
    ...
    it('should list all words whose part of speech does not start with given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.startsWith('v')).to.be.false
      )
    })
    // list pos not-ends m.
    it('should list all words whose part of speech does not end with given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.endsWith('m.')).to.be.false
      )
    })
    ...
    it('should list all words whose part of speech does not contain given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.PartOfSpeech.includes('vin.')).to.be.false
      )
    })
    // list pos not-is v.
    ...
    it('should list all words whose part of speech is not like the given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach((w) => {
        expect(w.data.Navi.startsWith('v') && w.data.Navi.endsWith('.')).to.be
          .false
      })
    })
    // list word starts ft
    ...
    it('should list all words that include given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.Navi.toLowerCase().includes('ts')).to.be.true
      )
    })
    // list word like f%w
    it('should list all words that are like given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(w.data.Navi.match(/f.*w/g).length > 0).to.be.true
      )
    })
    ...
    it('should list all words that are not like given string', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach((w) => {
        expect(w.data.Navi.startsWith('f') && w.data.Navi.endsWith('w')).to.be
          .false
      })
    })
    // list words first 20
    it('should list first 20 words released', () => {
      ...
      results.forEach(
        (w, i) => expect(w.data.ID === dictionary[i].data.ID).to.be.true
      )
    })
    // list words last 30
    it('should list last 30 words released', () => {
      ...
      results.forEach(
        (w, i) =>
          expect(w.data.ID === dictionary.slice(-30)[i].data.ID).to.be.true
      )
    })
    // list syllables > 1
    ...
    it('should list all words with stressed syllable position -1', () => {
      ...
      expect(results.length).to.be.greaterThan(0)
      results.forEach(
        (w) => expect(+w.data.Stressed === w.syllableCount()).to.be.true
      )
    })
    // list stress >= 1
    it('should list all words with stressed syllable position >= 1', () => {
